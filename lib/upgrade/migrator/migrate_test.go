...
var _ = Describe("UT for checking the version for migration.", func() {
	...
	DescribeTable("Checking canMigrate.",
		func(ver string, expectMigrate bool, expectError bool) {
			migrate, err := versionRequiresMigration(ver)
			Expect(migrate).To(Equal(expectMigrate))
			if expectError {
		...
		Entry("Expect v2.6.3 to not migrate", "v2.6.3", false, false),
		Entry("Expect v2.6.3-rc1 to not migrate", "v2.6.3-rc1", false, false),
		Entry("Expect v2.6.3-15-g0986c6cd to not migrate", "v2.6.3-15-g0986c6cd", false, false),
		...
		Entry("Expect v3.0 to not migrate", "v3.0.0", false, true),
	)
})
