...
// resourceKey is an entry in our cache.  It groups the a key with the last known
// revision that we processed.
...
func (wc *watcherCache) run() {
	...
	for {
		...
		wc.logger.WithField("RC", rc).Info("About to read from results channel")
		event := <-rc
		switch event.Type {
...
func (wc *watcherCache) resyncAndCreateWatcher() {
	...
	for {
		...
		// syncer.
		wc.logger.Info("Starting watch resync processing")
		...
		if wc.resourceType.UpdateProcessor != nil {
			...
			wc.resourceType.UpdateProcessor.SyncStarting()
		}
		...
		if err != nil {
			wc.logger.WithError(err).Debug("Failed to perform list")
...
// dataIsNowSynced handles processing to finish synchronization.
...
func (wc *watcherCache) dataIsNowSynced() {
...
// handleWatchEvent handles a watch event converting it required and fanning out
...
func (wc *watcherCache) handleAddedOrModifiedUpdate(kvp *model.KVPair) {
	...
	wc.validateResource(thisKeyString)
...
// validateResource validates a resource that is currently in the oldResources.
func (wc *watcherCache) validateResource(resourceKey string) {
