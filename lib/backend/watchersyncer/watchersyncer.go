...
type SyncerUpdateProcessor interface {
	...
	// SyncStarting is called when syncer is starting a full sync for the associated resource
...
func (rs *watcherSyncer) run() {
	log.Info("Running - sending status update")
	rs.sendStatusUpdate(api.WaitForDatastore)
	log.Info("Starting watcher caches")
	for _, wc := range rs.watcherCaches {
		go wc.run()
	}

	log.Info("Starting main watch processing loop")
