...
func (fc *fakeConverter) SyncStarting() {
}
...
func newWatcherSyncerTester(l []watchersyncer.ResourceType) *watcherSyncerTester {
	...
	// Create the syncer receiver.  The channels should be sufficiently large to handle
	// the full set of updates that we are comparing against.
	sr := &syncerReceiver{
		status:      api.SyncStatus(9),
		updates:     make(chan api.Update, 100),
		onUpdates:   make(chan []api.Update, 100),
		parseErrors: make(chan parseError, 10),
	}
	rst := &watcherSyncerTester{
		fc:            fc,
		sr:            sr,
		watcherSyncer: watchersyncer.New(fc, l, sr),
		lws:           lws,
	}
...
func (rst *watcherSyncerTester) expectStatus(status api.SyncStatus) {
	log.Infof("Expecting status of %d (%s)", status, status)
	// The status may ben in the process of changing, but it should reach the expected
	// status and then remain there.
	Eventually(rst.sr.getStatus).Should(Equal(status))
	Consistently(rst.sr.getStatus).Should(Equal(status))
}

// Call to test the onUpdate events (without worrying about which specific
// onUpdate events were received).
// This removes all updates and onUpdate events from the receiver.
func (rst *watcherSyncerTester) expectUpdates(expected []api.Update) {
	log.Infof("Expecting updates of %v", expected)
	updates := []api.Update{}
	for i := 0; i < 100; i++ {
		select {
		case update := <-rst.sr.updates:
	...
	// Clear the onUpdates channel.  We don't need to validate that.
loop:
	for {
		select {
		case <-rst.sr.onUpdates:
		default:
			break loop
		}
	}
...
// Call to test the actual OnUpdate requests that were received.  This is
// only useful when the test is blocking the receive thread such that outstanding
// watch events will be collated into a single update.
// This removes the onUpdate events from the receiver.
func (rst *watcherSyncerTester) expectOnUpdates(expected [][]api.Update) {
	log.Infof("Expecting onUpdates of %v", expected)
	onUpdates := [][]api.Update{}
	for i := 0; i < 100; i++ {
		select {
		case onUpdate := <-rst.sr.onUpdates:
			onUpdates = append(onUpdates, onUpdate)
		default:
			time.Sleep(10 * time.Millisecond)
		}
	}
	Expect(onUpdates).To(Equal(expected))
	// Clear the updates channel.  We don't need to validate that.
loop:
	for {
		select {
		case <-rst.sr.updates:
...
// Call to test the parse errors that were expected.
// This removes the parse error from the receiver.
func (rst *watcherSyncerTester) expectParseError(key, value string) {
	log.Infof("Expecting parse error: %v=%v", key, value)
	ps := parseError{}
	for i := 0; i < 10; i++ {
		select {
		case ps = <-rst.sr.parseErrors:
			return
		default:
		}
	}
	Expect(ps.rawKey).To(Equal(key))
	Expect(ps.rawValue).To(Equal(value))
}
...
func (rst *watcherSyncerTester) sendEvent(r watchersyncer.ResourceType, event api.WatchEvent) {
	...
	// We cannot send any events while the previous watcher is terminating.
	...
	// If the watcher is terminated then wait for the termination to complete before
	// sending the events (because we share a comment channel for sending the results).
...
func (rst *watcherSyncerTester) setBlockUpdateHandling(block bool) {
...
type syncerReceiver struct {
	...
	onUpdates   chan []api.Update
	updates     chan api.Update
	parseErrors chan parseError
	wg          sync.WaitGroup
}

// Store our current status.
...
// Queue the updates in both our updates channel (individual updates), and our onUpdates
// channel (this whole update).
func (s *syncerReceiver) OnUpdates(updates []api.Update) {
	...
	// We've updated the data and released the lock, but the test may need to block.
	s.wg.Wait()
	log.Info("OnUpdates processing complete")
}

// Queue the parse failed error.
func (s *syncerReceiver) ParseFailed(rawKey string, rawValue string) {
	log.WithFields(log.Fields{
		"Key":   rawKey,
		"Value": rawValue,
	}).Info("ParseFailed called")
	s.parseErrors <- parseError{rawKey: rawKey, rawValue: rawValue}
}
